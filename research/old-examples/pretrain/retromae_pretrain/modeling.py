...
class RetroMAEForPretraining(nn.Module):
	...
	def forward(self, encoder_input_ids, encoder_attention_mask, encoder_labels,
				decoder_input_ids, decoder_attention_mask, decoder_labels, eval: bool = False):
		lm_out: MaskedLMOutput = self.lm(encoder_input_ids, encoder_attention_mask, labels=encoder_labels,
										 output_hidden_states=True, return_dict=True)

		cls_hiddens = lm_out.hidden_states[-1][:, :1]  # B 1 D  获取cls向量表示
		# decode的输入做embedding
		...
		# decoder_attention_mask:(torch.Size([2, 77, 77]),  matrix_attention_mask: torch.Size([2, 1, 77, 77]))
		...
		if eval:
			return (loss + lm_out.loss, lm_out.hidden_states[-1][:, :1] )
		else:
